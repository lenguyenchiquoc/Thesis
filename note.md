...
- **[ĐÃ CẬP NHẬT — xem checkbox `[x]` bug base64-padding bên dưới]** Số liệu ban đầu (trước fix): 6/22 payload thật (Java-gzip, Ruby-gzip, NodeJS-gzip-qua-header, Wrapper-gzip, YAML-gzip-qua-header, Pickle) bị mất do bug modulo `%4==0` sau `.rstrip('=')`. **Sau khi fix base64-shape check** (`Utility/signatures.py`, tính lại padding đúng + thử decode thật thay vì đoán): cả 6/6 case này đã detect đúng, verify lại bằng `comprehensive.har`. Còn lại **3/22 (14%) false negative** do bug `raw_body` whitelist chưa fix (`JMSInvokerServlet`, `update-profile`, `config`).
